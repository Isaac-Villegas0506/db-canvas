...
const SidebarItem = ({ type, icon: Icon, label }: { type: string; icon: any; label: string }) => {
    ...
    return (
        <div
            className="flex flex-col items-center gap-2 p-3 bg-white dark:bg-zinc-900 rounded-lg border border-slate-200 dark:border-zinc-700/50 shadow-sm cursor-grab active:cursor-grabbing hover:border-blue-500 dark:hover:border-blue-400 hover:shadow transition-all group relative"
            draggable
            onDragStart={(e) => {
...
export const Sidebar: React.FC = () => {
    ...
    return (
        <div className="fixed sm:absolute bottom-6 sm:bottom-auto sm:top-20 left-1/2 sm:left-4 -translate-x-1/2 sm:translate-x-0 z-50">
            {!isSidebarOpen && (
                <button
            ...
            {isSidebarOpen && (
                <div className="flex flex-row sm:flex-col gap-3 p-2 sm:p-0 bg-white/90 sm:bg-transparent dark:bg-zinc-900/90 sm:dark:bg-transparent rounded-xl sm:rounded-none border sm:border-none border-slate-200 dark:border-zinc-700/50 shadow-xl sm:shadow-none backdrop-blur-sm sm:backdrop-blur-none sm:w-24">
                    ...
                    <button
                        onClick={toggleSidebar}
                        className="flex items-center justify-center p-2 bg-white dark:bg-zinc-900 rounded-lg border border-slate-200 dark:border-zinc-700/50 shadow-sm hover:bg-slate-50 dark:hover:bg-zinc-800 transition-all"
                        title="Ocultar panel de elementos"
                    >
                        <ChevronLeft size={16} className="text-slate-400 dark:text-slate-500 -rotate-90 sm:rotate-0" />
                    </button>
                </div>
            )}
        </div>
